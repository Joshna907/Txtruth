...
The web demo is Devnet-only. It uses Wallet Standard and `@solana/kit` in the browser, where the connected wallet retains all private-key and signing control. It includes a server-side RPC proxy for read-only signature inspection. The proxy calls Solana's `getSignatureStatuses` with transaction-history search enabled and `getTransaction` for execution metadata and the exact fee. Its endpoint is fixed to Devnet to avoid arbitrary-server requests and accidental Mainnet usage.
